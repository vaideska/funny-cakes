import React, { useCallback, Dispatch, SetStateAction, MouseEvent, useState } from "react";
...
export const CreateInstructionListContainer = ({ setInstructionList, instructionList, isEditForm, isLoadFile, setIsLoadFile }: CreateInstructionListContainerProps) => {
  ...
  const handleDeleteClick = useCallback((id: number) => (e: MouseEvent) => {
    ...
    setInstructionList((prev) => {
      const newState: RecipeInstruction[] = prev.filter((instruction, index) => index !== id);
      return newState;
    });
  }, [setInstructionList]);
