...
export const CreateInstructionList = ({ 
  setInstructionList, instructionList, handleAddClick, 
  handleDeleteClick, isEditForm, isLoadFile, 
  setIsLoadFile, selectedFiles, setSelectedFiles }: CreateInstructionListProps) => {
              ...
              <Grid container>
                ...
                {countInstruction > 1 ? 
                  <Grid item xs={12} sm={0.5}>
                    <IconButton aria-label="deleteInstruction" onClick={handleDeleteClick(index)} sx={{mt: 1}} disabled={!isEditForm}>< RemoveCircleOutlineOutlinedIcon/></IconButton>
                  </Grid>
                : null }
                {index + 1 === countInstruction ? 
                  <Grid item xs={12} sm={0.5}>
                    <IconButton aria-label="addInstruction" onClick={handleAddClick} sx={{mt: 1}} disabled={!isEditForm}>< AddCircleOutlineOutlinedIcon/></IconButton>
                  </Grid>
                : null }</Grid>
                <Grid item xs={12}>
